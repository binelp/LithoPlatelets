...
for kk=1:N
    ...
    sizeML_lithoAngles{kk} = outputStruct.results.DP.measuredParticleList_ML'; %#ok<SAGROW>
end

averagesML_lithoAngles = cell2mat(cellfun(@(x)mean(x),sizeML_lithoAngles(:),'UniformOutput',false));
stdsML_lithoAngles = cell2mat(cellfun(@(x)std(x),sizeML_lithoAngles(:),'UniformOutput',false));
